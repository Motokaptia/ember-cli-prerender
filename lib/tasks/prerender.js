  ...
  return () => {
    ...
    if (startServer) {
      ...
      return startServer
         ...
         */
        .then(() =>
          fetchFromLocalServer(`${localServerUrl}/sitemap.xml`)
            .then((body) => {
              if (body !== 'not_found') {
                const regex = /id="sitemap\-xml">\n([^]*)\n<\/pre>/i;
                const found = body.match(regex);
                if (!found || found.length !== 2) {
                  throw new Error('sitemap.xml could not be processed.')
                }
                const xmlContent = `<?xml version="1.0" encoding="UTF-8"?>\n${found[1]}`;

                return saveFile(`${localServerUrl}/sitemap.xml`, xmlContent);
              }
            })
        )
        /**
         ...
         */
        .then(() =>
          fetchFromLocalServer(`${localServerUrl}/sitemap.txt`)
            .then((body) => {
              const regex = /id="sitemap\-txt">\n([^]*)\n<\/pre>/i;
              const found = body.match(regex);
              if (!found || found.length !== 2) {
                throw new Error('sitemap.txt could not be processed.')
              }
              const urls = found[1].split('\n');

              return saveFile(`${localServerUrl}/sitemap.txt`, found[1])
                .then(() => urls);
            })
        )
        /**
